...
mkdir -p "${HOME}/.ssh"
gpg --quiet --batch --yes --decrypt --passphrase=${SBC_GPG_PASSWORD} --output .github/avereon.keystore .github/avereon.keystore.gpg
...
export PRODUCT_DEPLOY_PATH=/opt/sbc/store/${SBC_RELEASE}/${SBC_PRODUCT}
...
echo "JAVADOC_DEPLOY_PATH=/opt/sbc/store/latest/${SBC_PRODUCT}" >> $GITHUB_ENV
